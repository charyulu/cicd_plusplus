...
echo -e "\n Resource group Deploy Status: $DEPLOY_STATUS"
...
GITLAB_PUBLIC_IP=$(az network public-ip update -g "${RESOURCE_GROUP_NAME}" -n ${GITLAB_PUBLIC_IP_RESOURCE_NAME} --dns-name "${GITLAB_PUB_IP_DNS_PREFIX}" --allocation-method Static | jq -r '[.dnsSettings, .ipAddress]')
echo "${GITLAB_PUBLIC_IP}"
