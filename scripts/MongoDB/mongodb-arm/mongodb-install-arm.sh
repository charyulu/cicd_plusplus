...
MONGO_PUBLIC_IP=$(az network public-ip update -g "${RESOURCE_GROUP_NAME}" -n ${MONGO_PUBLIC_IP_RESOURCE_NAME} --dns-name "${MONGO_PUB_IP_DNS_PREFIX}" --allocation-method Static | jq -r '[.dnsSettings, .ipAddress]')
echo "${MONGO_PUBLIC_IP}"
